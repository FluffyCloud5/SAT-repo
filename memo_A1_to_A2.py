...
def _():
    ...
    The ESRC is organised as multiple sector grids. Each sector grid is a rectangle containing 10 by 10 sectors. Each sector is a discrete navigable unit. Sectors are connected by reinforced intra-wing corridors, and sector grids are connected by inter-wing corridors. The sector grid can be seen below.

    The objective is to command CRUDY-1 (Corridor Reconnaissance and Utility Drone — Year 1)--an Autonomous System (AS)--to navigate the facility’s corridor network and transport recovered supply units to an extraction point while ensuring a safe extraction. Ideally, the route taken by the AS minimises traversal cost and maximises the number of supply units recovered.
...
def _():
    ...
    Movement Options:
    - Inter-wing Corridor: This is shown as the red dotted line connecting two sectors from two different sector grids, which allows for direct movement between those two sectors.
    - Intra-wing Corridor: an intra-wing corridor is the absence of a wall between two adjacent sectors in the same sector grid
    - Corridor: A corridor is an intra-wing corridor or an inter-wing corridor.
    ...
    3.  inter-wing corridors have a length of 4 and intra-wing corridors have a length of 1, to match the facility representation. this is not correlated to traversal cost.
    """)
    return
...
def _():
    ...
    VP = {v : {
        ...
        wing": String (name of the wing v is in), <br>
        "location": (Integer, Integer)}<br>
        | v ∈ V}
    ...
    - A walk taken by the AS that maximises the number of supply units extracted. It starts at an entry and ends at an exit. (as a list). The list for the walk is the sequence of locations of the vertices visited
    ...
    1. move(x: distance in x direction, y: distance in y direction) → None         # allows the AS to move 'x' distance in the x direction and 'y' distance in the y direction.
...
def _():
    mo.md(r"""
    Position<br>
    1. The position of on object is defined as a tuple (x,y) indicating coordinate location where 1 unit distance is equal to the length of 1 intra-wing corridor.
    ...
    Direction<br>
    1. The direction can be calculated from the vector going from one vertex to another
        $,\vec{v} = (\Delta x, \Delta y)$ and
        $\theta = tan(\Delta y/ \Delta x)$<br>
...
def _():
    ...
    **8. Array**<br>
    ...
    - List (of vertices traversed in walk).
...
def _(algorithms):
    #Choose Algorithm
    algorithm_input = mo.ui.tabs({_algo:"" for _algo in algorithms.keys()}, value = "Brute Force")
...
def _(table, table_options):
    mo.callout(mo.vstack([table_options,table]), kind = "info")
    return
...
def _():
    ...
    Please refer to the table in part C4, which was used to decide on an algorithm.
...
@app.cell(hide_code=True)
def _(default_title):
    scenario_fac_v3 = m_fac_Av3(30012009)
    scenario_fac_v3['exit_b'] = (1,0,3)
    draw_fac_v3(scenario_fac_v3, title = default_title(seed = 30012009), legend = False)
...
def _():
    ...
    Perhaps an improvement would be to wrap the inputs into a map to help hide the clunk and make the pseudocode and python code cleaner. This might look like:
